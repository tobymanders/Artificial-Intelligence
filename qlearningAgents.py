...
class QLearningAgent(ReinforcementAgent):
  ...
  def update(self, state, action, nextState, reward):
    ...
    """
    "*** YOUR CODE HERE ***"
    new = reward + self.discount * self.getValue(nextState)
    old = self.qvalue[(state, action)]
    self.qvalue[(state, action)] = (1 - self.alpha) * old + self.alpha * new

    if self.qvalue[(state, action)] > self.getValue(state):
      self.value[state] = self.qvalue[(state, action)]
